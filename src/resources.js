...
function loadResource(name, wiki, max, rate, tradable, stackable, color) {
    ...
    if (global.resource[name].max === -1 || global.resource[name].max === -2) {
        ...
        res_container = $(`<div id="res${name}" class="resource${global.settings.resBar[name] ? ` showBar` : ``}" v-show="display" :style="{ '--percent-full': (bar && max > 0 ? (amount/max)*100 : 0) + '%' }"><div><h3 class="res has-text-${color} bar" @click="toggle('${name}')">{{ name | namespace }}</h3><span id="cnt${name}" class="count">{{ amount | size }} / {{ max | size }} <span class="has-text-gray" style="font-size: 0.8em;" v-if="timeToFull">({{ timeToFull }})</span></span></div></div>`);
    }
    ...
    vBind({
        ...
        computed: {
            timeToFull() {
                ...
                if (this.diff > 0 && this.amount < this.max) {
                    return timeFormat((this.max - this.amount) / this.diff);
                }
                if (this.diff < 0 && this.amount > 0) {
                    return timeFormat(this.amount / -this.diff);
                }
                return '';
            }
        },
