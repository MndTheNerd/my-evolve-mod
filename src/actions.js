...
export function payCosts(c_action, costs) {
    if (global.cheats && global.cheats.enabled) {
        if (global.cheats.one_click_evolve && c_action.id && c_action.id.indexOf('evolution-') === 0) {
            return true;
        }
        if (global.cheats.god_mode) {
            return true;
        }
    }
...
export function checkAffordable(c_action, max, raw) {
    if (global.cheats && global.cheats.enabled && global.cheats.god_mode) {
        return true;
    }
    if (c_action.cost) {
