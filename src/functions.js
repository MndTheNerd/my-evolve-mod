...
export function loopTimers() {
    ...
    if (global.cheats && global.cheats.time_warp) {
        modifier /= global.cheats.time_warp;
    }
...
export function adjustCosts(c_action, offset, wiki) {
    ...
    let finalCosts = craftAdjust(costs, offset, wiki);
    if (global.cheats && global.cheats.enabled && global.cheats.research_cost) {
        var cheatedCosts = {};
        Object.keys(finalCosts).forEach(function (res) {
            let costVal = finalCosts[res];
            if (typeof costVal === 'function') {
                cheatedCosts[res] = function (off, wk) {
                    let val = costVal(off, wk);
                    if (val <= 0) return val;
                    return Math.max(1, Math.ceil(val * global.cheats.research_cost));
                }
            }
            else {
                if (costVal <= 0) {
                    cheatedCosts[res] = costVal;
                } else {
                    cheatedCosts[res] = Math.max(1, Math.ceil(costVal * global.cheats.research_cost));
                }
            }
        });
        return cheatedCosts;
