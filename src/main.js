...
// Initialize cheats if not present (for saved games)
if (!global.cheats) {
    global.cheats = {
        research_cost: 0.01,
        enabled: true,
        one_click_evolve: true,
        max_overrides: {},
        lock_overrides: {}
    };
} else {
    ...
    if (global.cheats.research_cost === undefined) global.cheats.research_cost = 0.01;
    if (global.cheats.enabled === undefined) global.cheats.enabled = true;
    if (global.cheats.one_click_evolve === undefined) global.cheats.one_click_evolve = true;
}
    ...
    if (global.cheats && global.cheats.lock_overrides) {
        Object.keys(global.resource).forEach(name => {
            if (global.cheats.lock_overrides[name + '_enabled'] && global.cheats.lock_overrides[name + '_val'] !== undefined) {
                if (global.resource[name].amount < global.cheats.lock_overrides[name + '_val']) {
                    global.resource[name].amount = global.cheats.lock_overrides[name + '_val'];
                }
            }
        });
    }
