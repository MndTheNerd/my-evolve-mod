...
export function loadTab(tab) {
    ...
    switch (tab) {
        ...
        case 'mTabCheats':
            {
                if (!global.settings.tabLoad) {
                    tagEvent('page_view', { page_title: `Evolve - Cheats` });
                }
                drawCheats();
            }
            break;
...
export function index() {
    ...
    $('body').append(`<div id="topBar" class="topBar">
        ...
        <span class="settings" style="padding: 0 0.5rem; border-left: 1px solid #ccc; cursor: pointer;" id="themeToggle" role="button">◑</span>
    </div>`);

    $('#themeToggle').on('click', function () {
        let current = global.settings.theme;
        let next = current === 'dark' ? 'light' : 'dark';
        global.settings.theme = next;
        $('html').removeClass();
        $('html').addClass(next);
        $('html').addClass(global.settings.font);
    });
    ...
    // Cheats Tab
    let cheats = $(`<b-tab-item :visible="true" value="mTabCheats">
        <template slot="header">
            Cheats
        </template>
        <div id="mTabCheats"></div>
    </b-tab-item>`);
    tabs.append(cheats);
        ...
        <div class="promoBar">
            <span class="left">
                <h1>
                    ...
                    by <span class="has-text-success">Demagorddon</span>
                    <span style="font-size: 0.6em; display: block;">Modded by <a href="https://mndthenerd.com" target="_blank" class="has-text-info">MndtheNerd</a></span>
                </h1>
            </span>
            <span class="right">
...
export function drawCheats() {
    if (!global.cheats.max_overrides) global.cheats.max_overrides = {};
    if (!global.cheats.lock_overrides) global.cheats.lock_overrides = {};
    clearElement($(`#mTabCheats`));
    var content = $(`
        ...
        methods: {
            setToMax(name) {
                if (global.resource[name].max > 0) {
                    global.resource[name].amount = global.resource[name].max;
                } else {
                    global.resource[name].amount += 1000000;
                }
            },
            updateMax(name, val) {
